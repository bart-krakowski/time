...
import type { CalendarCoreOptions, CalendarState, Event } from './calendar';
...
export class DatePicker<TEvent extends Event> extends CalendarCore<TEvent> {
  datePickerStore: Store<DatePickerState>;
  options: DatePickerOptions<TEvent>;

  constructor(options: DatePickerOptions<TEvent>) {
    super(options);
